function [ portfolio, newStateMatrix ] = augmentedMACO( decisionData, stateMatrix )

% @Author = Charles Lai
% augmentedMACO - Augmented Moving Average Crossover Strategy
...
%
%			   This strategy will attempt to augment the basic dual MACO strategy with stop-loss 
%			   measures to prevent whipsawing using my own unique filters.
...
%		In that it is very profitable to short uptrends and long downtrends. I have no clue why.
	...
	[volumeLead, volumeLag] = movavg(volume, 21, 55);
	% Using these variables, recalulate the appropraite postion for our portfolio
	% If the cross over occurs without volume taken into account, take a half position in the security
	if (leadMA(end) > lagMA(end))
		portfolio = .5;
	elseif (leadMA(end) < lagMA(end)
		portfolio = -.5;
	end
	% If our volume condition is valid, take a full position in the security
	if (leadMA(end) > lagMA(end) && volumeLead(end) > volumeLag(end))
		portfolio = 1;
	elseif (leadMA(end) < lagMA(end) && volumeLead(end) < volumeLag(end))
		portfolio = -1;
